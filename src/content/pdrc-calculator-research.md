...
| **Pre-computed lookup JSON** | Instant, offline, auditable batch physics | Fixed city list (we ship **84**; interpolation extends coverage) |

We chose **pre-computed tables** embedded in the client bundle—similar in spirit to other industry tools that cache simulation or physics outputs. Offline scripts compute **city × building type × roof condition** cells; the UI multiplies per-m² savings by **roof area** and applies **grid emission factors** for CO₂.
